...
class ReshapeParser(Parser):
    """Simplified parser for ONNX Reshape node"""
    ...
    def generate_layer_node_for_reshape(self):
        def get_layer_node_input_format(B, C, H, W, new_shape, node_mapping, nodes_outputs):
            """
            Generate the necessary dictionary items required for the LayerNode creation.
            """
            # Equation
            d = {}
            d["equation"] = "O[b][c][h][w] = I[b][c][h][w]"
            ...
            H = H
            W = W
            new_shape = new_shape  # New shape for the output tensor
        ...
        assert len(ia_dimension_shape) == len(oa_dimension_shape) == len(new_shape), "Input, output, and new shape dimensions must match"

        B = ia_dimension_shape[0]
        C = ia_dimension_shape[1]
        H = ia_dimension_shape[2]
        W = ia_dimension_shape[3]
        new_shape = oa_dimension_shape  # 您提供的输出形状
        ...
        node_attrs = get_layer_node_input_format(
            B, C, H, W, new_shape, node_mapping, self.nodes_outputs
        )
