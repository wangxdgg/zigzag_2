...
class RollParser(Parser):
    """Parser for ONNX Roll nodes into LayerNode."""
    ...
    def generate_layer_node_for_roll(self):
        def get_layer_node_input_format(ia_shape, oa_shape, shifts, node_mapping):
            """
            Generate the necessary dictionary items required for the LayerNode creation.
            """
            # Equation
            d = {}
            d["equation"] = "O[b][c][oy][ox] = I[b][c][(iy+shy) % IY][(ix+shx) % IX]"
            ...
            d["pr_loop_dim_size"] = {"shx": shifts[0], "shy": shifts[1]}
        ...
        # Find shifts in attrs
        shifts = get_attribute_ints_with_name("shifts", attrs, default=[0, 0])
        ...
        node_attrs = get_layer_node_input_format(
            ia_dimension_shape, oa_dimension_shape, shifts, node_mapping
        )
